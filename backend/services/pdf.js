/**
 * PDF Service
 * Generates a professional tax invoice / reference PDF using Puppeteer + Handlebars.
 */
 ...
 *
 * PURPOSE:
 * Generates invoice/reference PDF files using HTML + Puppeteer.
 * Supports three formTypes: 'ndis', 'aged_care', 'rental'.
 *
 * FLOW:
...
async function generateInvoice({ formType, formData, draftOrder }) {
  ...
  // shipping_line is null when shipping is free, not applicable, or not set
  // (rental draft orders never set a shipping_line, so this is null for them).
  ...
  // ── Resolve participant / client / enquirer name ─────────────────────────────
  // NDIS & Aged Care forms use participant_full_name / first_name+last_name.
  // Rental forms use orderer_name instead.
  const participantName =
    formData.participant_full_name ||
    formData.orderer_name ||
    `${formData.first_name || ''} ${formData.last_name || ''}`.trim();

  // ── Resolve address components ────────────────────────────────────────────
  // NDIS & Aged Care forms use suburb/state/postcode.
  // Rental forms use city/suburb_area/postcode instead — normalise both.
  const addrLine1    = formData.address_line1 || '';
  const addrLine2    = formData.address_line2 ? `, ${formData.address_line2}` : '';
  const addrCity     = formData.suburb || formData.city || '';
  const addrState    = formData.state  || formData.suburb_area || '';
  const addrPostcode = formData.postcode || '';

  // ── Resolve contact email/phone ───────────────────────────────────────────
  const contactEmail =
    formData.participant_email || formData.submitter_email || formData.email || '';
  const contactPhone =
    formData.delivery_phone || formData.submitter_phone || formData.phone ||
    formData.shipping_phone || '';

  // ── Bill-To block ──────────────────────────────────────────────────────────
  // For plan-managed NDIS: bill to the Plan Manager.
  // For all others: bill to the participant / client / rental enquirer.
  ...
  const billTo = isPlanManaged
    ...
    : {
        ...
        address: [
          addrLine1 + addrLine2,
          `${addrCity} ${addrState} ${addrPostcode}`.trim(),
        ].filter(Boolean).join(', '),
        email: contactEmail,
        phone: contactPhone,
      };
  ...
  const rentalData = formType === 'rental' ? {
    productTitle:      (draftOrder.line_items && draftOrder.line_items[0] && draftOrder.line_items[0].title) || '',
    ordererRole:        formData.orderer_role       || '',
    deliveryTimeframe:  formData.delivery_timeframe || '',
    hasNdis:             formData.has_ndis === 'Yes' ? 'Yes' : 'No',
  } : {};
  ...
  const billingType =
    formType === 'ndis'   ? 'NDIS'            :
    formType === 'rental' ? 'Rental Enquiry'  :
                             'Aged Care';
  ...
  const templateData = {
    ...
    // shipping is null when free/not applicable/not set; the template hides the row.
